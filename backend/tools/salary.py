...
# ── Plausibility bounds for an ANNUAL salary in INR ───────────────────────────
# Anything outside this range is almost certainly a false positive
# (tax form codes like 1099/W2, version numbers, years, percentages, etc.)
_MIN_PLAUSIBLE_INR = 150_000        # ₹1.5L/year floor
_MAX_PLAUSIBLE_INR = 500_000_000    # ₹50Cr/year ceiling

# Number tokens that look like money but never are. Checked against the raw match.
_FALSE_POSITIVE_TOKENS = {"1099", "401", "401k", "w2", "w-2", "1040", "501c3", "24x7", "24/7"}


def _is_plausible(low: float | None, high: float | None) -> bool:
    """True only if the parsed salary falls in a believable annual-INR range."""
    if low is None or high is None:
        return False
    return _MIN_PLAUSIBLE_INR <= low <= _MAX_PLAUSIBLE_INR \
       and _MIN_PLAUSIBLE_INR <= high <= _MAX_PLAUSIBLE_INR
...
_HARDCODED_PATTERNS = [
    ...
    # USD: require an explicit $ sign. "$120,000 — $150,000" or "$120k-$150k"
    # We do NOT match bare "120000 USD" because plain numbers like "1099" (tax form)
    # get mistaken for dollars. The $ sign is the reliable signal.
...
def _parse_hardcoded(text: str) -> tuple[float | None, float | None]:
    ...
    for pattern, kind in _HARDCODED_PATTERNS:
        ...
        try:
            ...
            elif kind in ("usd", "usd_range", "usd_single"):
                # Apply k-suffix: "$120k" → 120,000
                span = m.group(0)
                if re.search(r"\d\s*k", span, re.I):
                    if low < 10_000:  low *= 1000
...
def extract_salary_inr(text: str) -> tuple[float | None, float | None]:
    ...
    if low is not None and _is_plausible(low, high):
        return low, high
    ...
    for m in _SALARY_RE.finditer(text):
        try:
            ...
            # Skip known false-positive tokens (401k, 1099, W2, years, etc.)
            raw_match = m.group(0).lower().replace(" ", "")
            if any(tok in raw_match for tok in _FALSE_POSITIVE_TOKENS):
                continue
            # Skip 4-digit years (1990–2099) with no currency symbol
            if re.fullmatch(r"(19|20)\d{2}", cleaned) and not (m.group("currency") or m.group("currency2")):
                continue
            ...
            # A bare number with a "k" suffix but NO currency is too risky
            # (e.g. "401k", "10k users") — require an explicit currency for k-numbers
            if m.group("low_k") and not (m.group("currency") or m.group("currency2")):
                continue
    ...
    # Final plausibility gate — reject junk (tax codes, version numbers, years)
    if not _is_plausible(best_low, best_high):
        return None, None
